...
private BST recursiveFromPostOrder(value[] nodes, int leftIndex, int rightIndex) {
    ...
    // It's a post-order traversal, so the root of the tree is in the last position
    value rootval = nodes[rightIndex];
    // Construct the root node, the left and right subtrees are then constructed
    // in recursive calls, after finding their extent
...
private int findLastSmaller(value[] nodes, int first, int last, int cut) {
    ...
    // invariants: nodes[low] < cut (since cut is the root value and a BST has no dupes)
